<template>
    <div>
        <label
            :for="id"
            class="flex w-full cursor-pointer items-center justify-center rounded-md border border-gray-300 bg-white px-2 py-3 text-center text-sm font-medium text-gray-700 hover:bg-gray-50 md:text-base"
        >
        ...
        <input :id="id" class="hidden" type="file" multiple @change="onFileChange" ref="fileInputRef" />

        <div class="mb-3 mt-2 text-xs text-gray-500 md:text-sm">* 최대 {{ maxFiles }}개, 파일당 최대 {{ maxSizeMb }}MB까지 첨부 가능합니다.</div>

        <div v-if="modelValue.length > 0" class="mt-4 space-y-2">
            <ul class="divide-y divide-gray-200 rounded-md border border-gray-200">
                <li v-for="(file, index) in modelValue" :key="file.name + index" class="flex items-center justify-between p-3 text-sm md:text-base">
                    <div class="flex-1 truncate">
                        <span class="font-medium text-gray-900">{{ file.name }}</span>
                        <span class="ml-2 text-gray-500">({{ formatBytes(file.size) }})</span>
                    </div>
                    <button @click.prevent="removeFile(index)" class="ml-4 font-medium text-red-600 hover:text-red-800">삭제</button>
                </li>
            </ul>
            <div class="text-right text-sm font-medium text-gray-700">총 {{ modelValue.length }}개 / {{ maxFiles }}개 선택</div>
        </div>
    </div>
</template>
...
function onFileChange(event: Event) {
    ...
    if (oversizedFiles.length > 0) {
        alert(`다음 파일의 용량이 ${props.maxSizeMb}MB를 초과합니다:\n- ${oversizedFiles.join('\n- ')}`)
    }
